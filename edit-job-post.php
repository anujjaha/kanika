...
{
	$jobId = $_GET['id'];
	$getJobInfo = "SELECT * FROM tbl_job_post where intJobPostId ='". $jobId ."'";
	$jobResult = mysql_query($getJobInfo);

	$jobInfo = mysql_fetch_array($jobResult);
}
...
if(isset($_POST['submit']))
{
	...
	if($result)
	{
		...
		$queryRemoveSkills = "DELETE FROM tbl_job_post_skill WHERE intJobPostId = '". $jobPostId ."'";
		mysql_query($queryRemoveSkills);

		$queryRemoveZip = "DELETE FROM tbl_job_post_multizipcode WHERE intProjectId = '". $jobPostId ."'";
		mysql_query($queryRemoveZip);
        ...
        {
        	if(isset($multiZip[$i]) && !empty($multiZip[$i]))
        	{
		...
		while($Row_Zip_Code = mysql_fetch_array($Result_Zip_Code))
		{
		   /*********** mail template for user *************/
			...
			$message = '<html>
				<head>
				<title> Notification Project Updated</title>
				</head>
				<body>
...
?>
	<script>
		window.location.assign('view-projects.php');
	</script>
<?php
}
	   ...
	   <form action="" method="post" id="my_form" novalidate   enctype="multipart/form-data">
		...
		<div class="col-md-12">
			   ...
			   <div class="col-lg-6 confirm_line">
				...
				<div class="col-lg-8 col-md-8 col-sm-8 col-xs-12">
					<div class="confirm1">
						...
						<?php
							...
							{ 
								$selected = '';
								if($jobInfo['vchJobWorkType'] == $row_category['intCategoryID'])
								{
				...
				<div class="col-lg-6 confirm_line">
					...
					<div class="col-lg-8 col-md-8 col-sm-8 col-xs-12">
					  <div class="confirm1">
					    <select id="subCategories" name="sub_category">
					    	<?php
							$subCategories = "select * from child_categories WHERE category_id = '".$jobInfo['vchJobWorkType']."'";

							$resultSubCategories = mysql_query($subCategories);
							while($rowSubCategory = mysql_fetch_array($resultSubCategories))
							{ 
								$selected = '';
								if(in_array( $rowSubCategory['id'], explode(",", $jobInfo['vchJobWorkSubType'])))
								{
									$selected = "selected='selected'";
								}
						?>
							<option name="catSearch" <?php echo $selected;?> class="catSearch"value="<?php echo $rowSubCategory['id']; ?>">
								<?php echo $rowSubCategory['title'];?>
							</option>
						<?php 
							} 
						?>
	          	...
	          	<div class="col-lg-6">
		            ...
		            <div class="col-lg-8 col-md-8 col-sm-8 col-xs-12">
		              <div class="confirm1">
		                <input name="jobtitle" id="jobtitle" type="text" value="<?php echo $jobInfo['vchJobPostTitle'];?>"  placeholder="please enter job title" required="true" />
		              </div>
	            	</div>
	          	</div>
	              ...
	              <div class="confirm1">
	              	<div class="input_fields_wrap">
						<div>
							<div class="confirm1">
							<?php
								$getAllZipCode = 'SELECT intMultizipCode FROM tbl_job_post_multizipcode WHERE intProjectId = "'. $jobInfo['intJobPostId'] .'"';
								$zipCodeResult = mysql_query($getAllZipCode);
								while($rowZip = mysql_fetch_array($zipCodeResult))
								{
							?>
								<input name="job_zip_code[]" id="job_zip_code" type="text" value="1<?php echo $rowZip['intMultizipCode'];?>"  required="true" />
							<?php
								}
							?>
							</div>
	            ...
	            <div class="col-lg-8 col-md-8 col-sm-8 col-xs-12">
	              <div class="confirm1">
					<select data-placeholder="please enter job skill" class="chosen-select" multiple  name="job_Skill[]"  id="job_Skill" required="true">
						<?php 
						$getSkills = "SELECT intSkillID from tbl_job_post_skill WHERE intJobPostId ='" .$jobInfo['intJobPostId']. "'";
						$getSkillResult = mysql_query($getSkills);
						$skillSet = array();
						while($rowSkill = mysql_fetch_array($getSkillResult))
						{
							$skillSet[] = $rowSkill['intSkillID'];
						}

							while($Row_skill = mysql_fetch_array($Result_skill))
							{
								$selected = "";
								if(in_array($Row_skill['intSkillId'], $skillSet))
								{
									$selected = 'selected="selected"';
								}
						?>
							<option <?php echo $selected;?> value="<?php echo $Row_skill['intSkillId'];?>">
            ...
            <div class="col-lg-8 col-md-8 col-sm-8 col-xs-12" id="dddd">
              	<input type="radio" name="jobprice" <?php if($jobInfo['bidpricetype'] == "Fixed Price") { echo "checked='checked'"; }?> value="Fixed Price" class="jobprice" id="SetFixedPrice" required="true" />Set Fixed Price
				<input type="radio" name="jobprice" <?php if($jobInfo['bidpricetype'] == "Fixed Price") { echo "checked='checked'"; }?> value="Hourly Rate" class="jobprice" id="SetAnHourlyRate"required="true" />Set An Hourly Rate
			</div>
		  ...
		  <div class="col-lg-12 confirm_line">
            ...
            <div class="col-lg-10 col-md-10 col-sm-10 col-xs-12">
              <div class="confirm1">
                <textarea rows="10" cols="80" name="job_post_desc" id="job_post_desc" required="true" placeholder="please enter job description"><?php echo $jobInfo['vchJobPostDescription'] ;?></textarea> 
				<script type="text/javascript">
			...
			<div class="col-lg-6 confirm_line">
				<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
				  <div class="confirm">
				  	<input type="hidden" name="job_post_id" value="<?php echo $jobInfo['intJobPostId'];?>">
					<input name="submit" id="submit" type="submit" value="Update Job" class="btn btn-primary" />
				  </div>
				</div>
			  </div>
        </div>
